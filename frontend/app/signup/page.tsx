  ...
  return (
    <main className="flex min-h-screen flex-col md:flex-row">
      ...
      <section className="flex flex-col justify-between bg-deep p-8 md:w-[560px] md:p-16">
        ...
        <p className="hidden text-[12px] text-light/70 md:block">
          ENGI 4503 · Analytics in Python
        </p>
      </section>
        ...
        <div className="flex w-full max-w-[640px] flex-col gap-6 rounded-[20px] border border-line bg-surface p-6 md:p-11">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <Field
              ...
              hint={usernameState === "ok" ? "Available." : undefined}
            >
              <Input
                value={username}
                ...
                state={
                  usernameState === "taken" ? "error" : usernameState === "ok" ? "ok" : "default"
                }
              />
            </Field>
            ...
            <Field
              label="Phone number"
              optional
              hint="Only if you want buyers to text you. Email always works."
            >
              <Input value={phone} onChange={setPhone} placeholder="+1 (646) 555-0142" />
            </Field>
          </div>

          <div className="grid gap-5 md:grid-cols-2">
            <Field label="Nationality">
              <Input value={nationality} onChange={setNationality} placeholder="KR" />
            </Field>
            <Field label="College / School">
              <Input value={school} onChange={setSchool} placeholder="cbs" />
            </Field>
          </div>
                ...
                <li key={z.zip_code}>
                  ...
                  >
                    ...
                    <span className="flex-1">
                      ...
                      <span className="block text-[11.5px] text-ink2">{z.borough}, NY</span>
                    </span>
                    <span className="text-[11.5px] text-ink3">{z.miles_away} mi</span>
                  </button>
                </li>
              ))}
            </ul>
          ...
          <Button full disabled={missing.length > 0} onClick={submit}>
            Create account
          </Button>
          {missing.length > 0 && (
