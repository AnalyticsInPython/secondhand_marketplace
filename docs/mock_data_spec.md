...
always gets the same colour. `listing_photos.url` is `/photos/<listing_id>/<position>.webp` (root-relative).
...
**Name the columns.** `\copy` maps by *position* and ignores the header, so a
table whose columns were declared in a different order silently loads the wrong
values into the wrong fields. Spelling them out costs nothing and removes the
whole class of problem:

```sql
\copy zip_reference (zip_code,neighbourhood,borough,lat,lon,miles_from_campus) from 'data/zip_reference.csv' with (format csv, header, null '');
\copy users (id,email,username,display_name,phone,phone_contact_enabled,nationality,school,grade,zip_code,default_radius_mi,default_filter_same_zip,default_filter_same_nationality,default_filter_same_school,is_verified,status,created_at,updated_at) from 'data/users.csv' with (format csv, header, null '');
\copy listings (id,seller_id,source,title,description,category,subcategory,condition,price_cents,is_free,is_negotiable,zip_code,status,view_count,save_count,enquiry_count,external_url,posted_at,sold_at) from 'data/listings.csv' with (format csv, header, null '');
\copy listing_photos (id,listing_id,url,position,created_at) from 'data/listing_photos.csv' with (format csv, header, null '');
\copy listing_views (id,listing_id,viewer_id,surface,viewed_at) from 'data/listing_views.csv' with (format csv, header, null '');
\copy saves (id,listing_id,user_id,created_at) from 'data/saves.csv' with (format csv, header, null '');
\copy enquiries (id,listing_id,buyer_id,channel,created_at) from 'data/enquiries.csv' with (format csv, header, null '');
\copy filter_events (id,user_id,filter_key,value,result_count,created_at) from 'data/filter_events.csv' with (format csv, header, null '');
```

Load in that order — the foreign keys require it.

### Through the backend (what the README's run instructions use)

```bash
cd backend
python -m scripts.seed --reset                  # load data/*.csv as they stand
python -m scripts.seed --reset --regenerate     # rebuild the CSVs first, then load
python -m scripts.seed --reset --users 2000 --listings 3000
```

`backend/scripts/seed.py` is a **loader**, not a generator. It used to generate its
own rows, which meant two implementations of §9 that had already drifted apart —
different ZIP tables, grade and school drawn independently so undergraduates
appeared at CBS, and saves sampled from users who had never viewed the listing.
Everything about *how the data looks* now lives in `seed/`; that file only moves
...
| `email` | `text` UNIQUE | no | `{initials}{4 digits}@{school domain}`, matching §4.1's regex. The domain is the one the member's school issues — CBS on `gsb.columbia.edu`, Teachers College on `tc`, Mailman and VP&S on `cumc`, everyone else on plain `columbia.edu` — so all four admitted domains are exercised by the data. Initials come from the **legal name**, not the handle: the design's own example pairs `dl3729@columbia.edu` with `@brian_dw` |
| `username` | `text` UNIQUE | no | Derived from the name, `[a-zA-Z0-9._]{3,20}`. Deliberately mixed shapes: `wei_zh`, `grace.miller`, `j.kim42`. The longest generated is 19 characters |
...
| `url` | `text` | no | `/photos/<listing_id>/<position>.webp` — root-relative, so it resolves identically on `/` and on `/listings/<id>` |
...
**There is one ZIP table and it lives in `backend/app/services/geo.py`.**
`seed/zips.py` loads it from there rather than keeping a copy. They used to be
separate, and had drifted to 18 codes against 47 — which put 26% of generated
listings in ZIPs the API rejected, invisible to every radius query rather than
failing loudly, because `geo.zips_within()` resolves the radius to a ZIP list.
Add a ZIP in `geo.py` and nowhere else.
...
|---|---|---|
| 1 | `email` UNIQUE, on one of the four admitted domains — matched on the **whole domain**, never a suffix (`endswith("@columbia.edu")` rejects `@gsb.columbia.edu`; a loose suffix test admits `@evil-columbia.edu`) | §4.1, `backend/app/emails.py` |
...
| `backend/scripts/seed.py` | Loads `data/*.csv` into the database. Not a generator |
| `backend/app/services/geo.py` | **The** ZIP table. `seed/zips.py` reads it from here |
...
unrepresentable. The API now agrees: `routers/listings.py` compares the *seller's*
ZIP for `same_zip`, matching what §5.3 computes the badge from. It previously
compared `Listing.zip_code`, which also let external listings — a pickup ZIP with
no seller behind it — pass a trust filter; 10 eBay and Facebook rows slipped into
a trust-filtered feed on this corpus.

If the product genuinely wants people listing from an office or a friend's place,
say so and I will unlock it — but the badge rule should then be restated in terms
of one column or the other.
...
1. **Real ZIP centroids** from the US Census ZCTA gazetteer, replacing the
   approximations in `backend/app/services/geo.py` (good to ~0.1 mi, which is the
   precision §5.2 rounds to, but they are still hand-entered).
